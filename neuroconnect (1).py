...
Instale: pip install customtkinter
"""
import customtkinter as ctk
import tkinter as tk
from tkinter import messagebox
import json, os, threading, time, uuid, hashlib
from datetime import datetime

# ══════════════════════════════════════════════════════════════════════════════
#  PALETAS DE CORES  (5 opções para o usuário escolher)
# ══════════════════════════════════════════════════════════════════════════════
TEMAS = {
    "Bege Suave": {
        "fundo":      "#F5EDD8",
...
T = dict(TEMAS[NOME_TEMA_PADRAO])   # tema ativo (mutável em tempo de execução)

# ── Cores fixas de destaque (não mudam com tema) ──────────────────────────────
LARANJA     = "#F5893C"
LARANJA_ESC = "#D96E20"
VERDE_COR   = "#4CAF50"
VERDE_ESC   = "#388E3C"
...
# Paleta de cores para o usuário escolher na rotina
PALETA_CORES = [
    ("Laranja",  "#F5893C"),
    ("Verde",    "#4CAF50"),
    ("Roxo",     "#5C5FA6"),
...
def escurecer(hex_cor, fator=0.80):
    """Retorna uma versão mais escura da cor para o efeito de hover."""
...
# ══════════════════════════════════════════════════════════════════════════════
#  WIDGET: Logo (símbolo ∞ desenhado em canvas)
# ══════════════════════════════════════════════════════════════════════════════
...
class TelaLogin(ctk.CTkFrame):
    ...
    def _entrar(self):
        ...
        if u and u["senha"] == hash_senha(senha):
            tema_salvo = u.get("tema", NOME_TEMA_PADRAO)
            if tema_salvo in TEMAS:
                aplicar_tema(tema_salvo)
    ...
    def _criar_conta(self):
        ...
        jan.geometry("420x420")      # janela bem maior para caber tudo
        jan.grab_set()
...
class DialogoTarefa(ctk.CTkToplevel):
    ...
    def _construir(self):
        ...
        tk.Label(cab, text=titulo, bg=self._cor, fg="#FFFFFF",
...
class TelaPrincipal(ctk.CTkFrame):
    ...
    def _criar_barra_topo(self):
        ...
        # Botão de tema
        ctk.CTkButton(dir_, text="Tema", width=74, height=32,
                      fg_color=T["card"], hover_color=T["borda"],
                      text_color=T["texto"], font=("Helvetica", 11),
                      command=self._escolher_tema).pack(side="left", padx=4)

        ctk.CTkButton(dir_, text="+ Rotina", width=94, height=32,
                      fg_color=ROXO, hover_color=ROXO_ESC,
                      text_color="#FFFFFF", font=("Helvetica", 11),
                      command=self._nova_rotina).pack(side="left", padx=4)

        ctk.CTkButton(dir_, text="Sair", width=64, height=32,
                      fg_color=T["card"], hover_color=T["borda"],
                      text_color=T["texto"], font=("Helvetica", 11),
    ...
    def _escolher_tema(self):
        ...
        ctk.CTkLabel(jan, text="Escolha a paleta de cores",
                     ...
                     text_color=T["texto"]).pack(pady=(20, 10))

        tema_atual = self._dados_usuario().get("tema", NOME_TEMA_PADRAO)

        for nome_tema, cfg in TEMAS.items():
            selecionado = (nome_tema == tema_atual)

            linha = tk.Frame(jan, bg=cfg["superficie"],
                             relief="solid" if selecionado else "flat",
                             bd=2 if selecionado else 0,
                             cursor="hand2")
            linha.pack(fill="x", padx=24, pady=4)

            # Quadradinhos mostrando as cores do tema
            amos = tk.Frame(linha, bg=cfg["superficie"])
            amos.pack(side="left", padx=8, pady=6)
            for cor_hex in [cfg["fundo"], cfg["card"], cfg["borda"]]:
                tk.Frame(amos, bg=cor_hex, width=18, height=18).pack(
                    side="left", padx=2)

            tk.Label(linha, text=nome_tema, bg=cfg["superficie"],
                     fg=cfg["texto"], font=("Helvetica", 12, "bold"),
    ...
    # ── Nova rotina com escolha de cor ───────────────────────────────────────
    def _nova_rotina(self):
        ...
        # Label de preview da cor selecionada
        ...
        for i, (nome_cor, hex_cor) in enumerate(PALETA_CORES):
            linha_g = i // 5
            col_g   = i % 5
            btn = tk.Button(grade, bg=hex_cor, width=3, height=1,
                            relief="flat", cursor="hand2",
                            command=lambda h=hex_cor, n=nome_cor: selecionar_cor(h, n))
            btn.grid(row=linha_g, column=col_g, padx=5, pady=4)
    ...
    def _alerta(self, nome, minutos, horario):
        ...
        ctk.CTkLabel(jan, text="Proxima atividade!",
                     font=("Helvetica", 15, "bold"),
